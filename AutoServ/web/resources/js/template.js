...
$(document).ready(function () {
	/*
	 * регистрация обработчиков для композита и нет
	 * */
...
/**
 * Валидация для формы отзыва на главной странице
 * @returns {boolean}
 */
function replierValidity()
{
	...
	$("form[name='REPLIER']").validity(function() {
		$("#REVIEW_TEXT")
			.require('Поле "Комментрий" обязательно для заполнения');
		$("input[name='captcha_word'][type='text']").require('Введите символы с картинки')
			.maxLength(5,"Введено более 5 символов")
			.minLength(5,"Введено менее 5 символов");
		$("input[name='PROPERTY[316][0]'] [type='text']").require('Введите Email');
		$("input[name='PROPERTY[315][0]'] [type='text']").require('Введите Имя')
	});
	return true;
}
/**
 * Открывает / закрывает авторизацию в хедере
 * @returns {boolean}
 */
...
/**
 * Обработка кнопки записаться на ТО
 * @returns {boolean}
 */
...
jQuery(document).ready(function(){
	...
	/*==============Промобаннер-слайдер на главной===============*/
	$('.coin-slider').slides({
		preload: true,
	...
	/*===============Вставляет текст по-умолчанию в инпуты===============*/
	...
	/*===============Вызов плагина "Darkbox" (показывает полный размер изображения)===============*/
...
function SubmitAjaxForm (link)
{
	...
	$('#pop-form-form').submit(function(e){
        ...
        {
			$('.agree').remove();
            $('#pop-form-form .container_form .line_block:eq(0)').before('<font class="errortext agree">Вы не дали согласие на обработку Ваших персональных данных<br/><br/></font>');
            agree = false;
        }
